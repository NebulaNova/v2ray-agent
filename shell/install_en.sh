...
checkCentosSELinux() {
    if [[ -f "/etc/selinux/config" ]] && ! grep -q "SELINUX=disabled" <"/etc/selinux/config"; then
        echoContent yellow "# Important notes"
        echoContent yellow "SELinux has been detected to be enabled. Please manually disable it. The tutorial is as follows:"
...
checkSystem() {
    ...
    if [[ -z ${release} ]]; then
        echoContent red "\nThis script does not support this system, please provide the following log to the developer\n"
...
checkCPUVendor() {
    if [[ -n $(which uname) ]]; then
        ...
        echoContent red "Unable to recognize this CPU architecture, defaulting to amd64, x86_64--->"
        xrayCoreCPUVendor="Xray-linux-64"
        v2rayCoreCPUVendor="v2ray-linux-64"
    fi
}
    ...
    if [[ -n $(pgrep -f "BT-Panel") ]]; then
        ...
        if [[ -d '/www/server/panel/vhost/cert/' && -n $(find /www/server/panel/vhost/cert/*/fullchain.pem) ]]; then
            ...
            if [[ -n "${selectBTDomain}" ]]; then
                ...
                if [[ -z "${btDomain}" ]]; then
                    echoContent red " ---> Selection error, please choose again"
                    checkBTPanel
                else
                    domain=${btDomain}
...
checkUFWAllowPort() {
    if ufw status | grep -q "$1"; then
        echoContent green " ---> Port $1 opened successfully"
    else
...
installTools() {
    ...
    echoContent green " ---> Checking and installing updates. [The new machine may be slow. If there is no response for a long time, please manually stop and execute again.]"
    ...
    if [[ "${selectCustomInstallType}" == "7" ]]; then
        echoContent green " ---> Detected a service that does not require Nginx as a dependency, skipping installation"
    else
        if ! find /usr/bin /usr/sbin | grep -q -w nginx; then
            ...
            if [[ ${nginxVersion} -lt 14 ]]; then
                read -r -p "The current version of Nginx does not support gRPC, which will cause installation failure. Do you want to uninstall Nginx and reinstall it? [y/n]:" unInstallNginxStatus
    ...
    if [[ "${selectCustomInstallType}" == "7" ]]; then
        echoContent green " ---> Detected a service that does not require certificate dependencies, skipping installation"
    else
        if [[ ! -d "$HOME/.acme.sh" ]] || [[ -d "$HOME/.acme.sh" && -z $(find "$HOME/.acme.sh/acme.sh") ]]; then
            ...
            if [[ ! -d "$HOME/.acme.sh" ]] || [[ -z $(find "$HOME/.acme.sh/acme.sh") ]]; then
                ...
                echoContent red "1. Failed to retrieve Github file. Please wait for Github to recover and try again. You can check the recovery progress at [https://www.githubstatus.com/]."
                echoContent red "Bug found in the acme.sh script. You can check the issues at [https://github.com/acmesh-official/acme.sh]."
                echoContent red "3. If it is a pure IPv6 machine, please set up NAT64. You can execute the following command. If adding the following command is still not available, please try using a different NAT64."
...
checkDNSIP() {
    ...
    if echo "${dnsIP}" | grep -q "timed out" || [[ -z "${dnsIP}" ]]; then
        echo
        echoContent red " ---> Unable to retrieve the IPv4 address of the domain name through DNS"
        echoContent green " ---> Attempting to check the IPv6 address of the domain"
        dnsIP=$(dig @2606:4700:4700::1111 +time=1 aaaa +short "${domain}")
        type=6
        if [[ -z "${dnsIP}" ]]; then
            echoContent red " ---> Unable to obtain the IPv6 address of the domain name through DNS, exiting installation"
            exit 0
        fi
    fi
    local publicIP=
    ...
    if [[ "${publicIP}" != "${dnsIP}" ]]; then
        echoContent red " ---> Domain name resolution IP does not match the current server IP\n"
        echoContent yellow " ---> Please check if the domain name resolution is effective and correct."
...
EOF
    ...
    if [[ "${checkPortOpenResult}" == "fjkvymb6len" ]]; then
        ...
        echoContent green " ---> No open port detected on ${port}, exiting installation"
        if echo "${checkPortOpenResult}" | grep -q "cloudflare"; then
            echoContent yellow " ---> Please close the cloud and wait for three minutes before trying again."
        else
            if [[ -z "${checkPortOpenResult}" ]]; then
                echoContent red " ---> Please check if there is a web firewall, such as Oracle or other cloud service providers."
                echoContent red " ---> Check if you have installed nginx yourself and if there are any configuration conflicts. You can try reinstalling with a clean system to troubleshoot."
            else
                echoContent red " ---> Error log: ${checkPortOpenResult}, please submit this error log through issues for feedback."
            fi
        fi
        exit 0
    fi
...
initTLSNginxConfig() {
    ...
    if [[ -n "${currentHost}" ]]; then
        echo
        read -r -p "Read last installation record, do you want to use the domain name from the last installation? [y/n]:" historyDomainStatus
...
removeNginxDefaultConf() {
    if [[ -f ${nginxConfigPath}default.conf ]]; then
        if [[ "$(grep -c "server_name" <${nginxConfigPath}default.conf)" == "1" ]] && [[ "$(grep -c "server_name  localhost;" <${nginxConfigPath}default.conf)" == "1" ]]; then
            echoContent green " ---> Delete default Nginx configuration"
...
checkIP() {
    ...
    if [[ -z ${localIP} ]] || ! echo "${localIP}" | sed '1{s/[^(]*(//;s/).*//;q}' | grep -q '\.' && ! echo "${localIP}" | sed '1{s/[^(]*(//;s/).*//;q}' | grep -q ':'; then
        echoContent red "\n ---> No IP detected for the current domain"
        ...
        echoContent yellow " ---> 2. Check if the domain name's DNS resolution is correct"
        ...
        echoContent yellow " --->  4. If there is a problem with starting Nginx, please manually start Nginx to check for errors. If you are unable to resolve the issue on your own, please submit an issue."
        echo
        echoContent skyBlue " ---> If all the settings above are correct, please reinstall the clean system and try again."

        if [[ -n ${localIP} ]]; then
            echoContent yellow " ---> Exception detected in return value, it is recommended to manually uninstall nginx and then re-run the script"
            echoContent red " ---> Exception result: ${localIP}"
            exit 0
        fi
    else
        if echo "${localIP}" | awk -F "[,]" '{print $2}' | grep -q "." || echo "${localIP}" | awk -F "[,]" '{print $2}' | grep -q ":"; then
            echoContent red "\n ---> Multiple IPs detected, please confirm if Cloudflare's cloud is turned off"
            echoContent yellow " ---> After closing the cloud, wait for three minutes and then retry."
            echoContent yellow " ---> Detected IP addresses are as follows: [${localIP}]"
            exit 0
        fi
        #        echoContent green " ---> 当前域名ip为:[${localIP}]"
        echoContent green " ---> Checking if the current domain name IP is correct"
    fi
}
# 自定义email
customSSLEmail() {
    ...
    if [[ -d "/root/.acme.sh" && -f "/root/.acme.sh/account.conf" ]]; then
        if ! grep -q "ACCOUNT_EMAIL" <"/root/.acme.sh/account.conf" && ! echo "${sslType}" | grep -q "letsencrypt"; then
            ...
            if echo "${sslEmail}" | grep -q "@"; then
                ...
                echoContent yellow "Please enter a valid email format [e.g. username@example.com]"
                customSSLEmail
            fi
        fi
    fi
...
acmeInstallSSL() {
    if [[ "${dnsSSLStatus}" == "true" ]]; then
        ...
        if [[ -n "${txtValue}" ]]; then
            ...
            echoContent yellow " ---> If multiple machines are installed with wildcard certificates for the same domain name, please add multiple TXT records. There is no need to modify previously added TXT records."
            echoContent green " --->  name：_acme-challenge"
            echoContent green " --->  value：${txtValue}"
            echoContent yellow " ---> Please wait for 1-2 minutes after the addition is completed"
            echo
            read -r -p "Whether to add completed [y/n]:" addDNSTXTRecordStatus
...
customPortFunction() {
    ...
    if [[ -n "${customPort}" || -n "${currentPort}" ]]; then
        echo
        read -r -p "Read the port used during the last installation, do you want to use the port used during the last installation? [y/n]:" historyCustomPortStatus
    ...
    if [[ -z "${currentPort}" ]] || [[ "${historyCustomPortStatus}" == "n" ]]; then
        ...
        if [[ -n "${btDomain}" ]]; then
            echoContent yellow "Please enter the port number [cannot be the same as the BT Panel port, press Enter for random]"
            read -r -p "Port:" port
            if [[ -z "${port}" ]]; then
...
checkPort() {
    if [[ -n "$1" ]] && lsof -i "tcp:$1" | grep -q LISTEN; then
        echoContent red "\n ---> Port $1 is occupied, please manually close it before installation\n"
...
installTLS() {
    ...
    if [[ -f "/etc/v2ray-agent/tls/${tlsDomain}.crt" && -f "/etc/v2ray-agent/tls/${tlsDomain}.key" && -n $(cat "/etc/v2ray-agent/tls/${tlsDomain}.crt") ]] || [[ -d "$HOME/.acme.sh/${tlsDomain}_ecc" && -f "$HOME/.acme.sh/${tlsDomain}_ecc/${tlsDomain}.key" && -f "$HOME/.acme.sh/${tlsDomain}_ecc/${tlsDomain}.cer" ]]; then
        ...
        echoContent green " ---> Installing TLS certificate, requires dependency on port 80"
        allowPort 80
        ...
        if [[ ! -f "/etc/v2ray-agent/tls/${tlsDomain}.crt" || ! -f "/etc/v2ray-agent/tls/${tlsDomain}.key" ]] || [[ -z $(cat "/etc/v2ray-agent/tls/${tlsDomain}.key") || -z $(cat "/etc/v2ray-agent/tls/${tlsDomain}.crt") ]]; then
            ...
            if tail -n 10 /etc/v2ray-agent/tls/acme.log | grep -q "Could not validate email address as valid"; then
                echoContent red " ---> Email failed to pass SSL vendor verification, please re-enter"
                echo
        ...
        echoContent green " ---> TLS generation successful"
    else
...
# 自定义/随机路径
randomPathFunction() {
    echoContent skyBlue "\nProgress $1/${totalProgress}: Generating random path"

    if [[ -n "${currentPath}" ]]; then
        echo
        read -r -p "Read the previous installation record, do you want to use the path from the previous installation? [y/n]:" historyPathStatus
        echo
    fi
    ...
    if [[ "${historyPathStatus}" == "y" ]]; then
        ...
        echoContent yellow "Please enter a custom path [e.g. alone], without slashes. Press [Enter] for a random path."
        ...
        if [[ -z "${customPath}" ]]; then
            ...
            if [[ "${customPath: -2}" == "ws" ]]; then
                echo
                echoContent red " ---> Custom path cannot end with 'ws', otherwise the split path cannot be distinguished"
...
# Nginx伪装博客
nginxBlog() {
    echoContent skyBlue "\nProgress $1/${totalProgress} : Adding camouflage site"
    if [[ -d "${nginxStaticPath}" && -f "${nginxStaticPath}/check" ]]; then
        echo
        read -r -p "Detected installation of disguised website, do you need to reinstall [y/n]:" nginxBlogInstallStatus
...
updateSELinuxHTTPPortT() {
    ...
    if find /usr/bin /usr/sbin | grep -q -w semanage && find /usr/bin /usr/sbin | grep -q -w getenforce && grep -E "31300|31302" </etc/v2ray-agent/nginx_error.log | grep -q "Permission denied"; then
        echoContent red " ---> Checking if SELinux port is open"
...
handleNginx() {
    ...
    if [[ -z $(pgrep -f "nginx") ]] && [[ "$1" == "start" ]]; then
        ...
        if [[ -z $(pgrep -f "nginx") ]]; then
            ...
            echoContent red " ---> Please manually try installing nginx and then run the script again."
        ...
        echoContent green " ---> Nginx shutdown successful"
    fi
}
...
installCronTLS() {
    if [[ -z "${btDomain}" ]]; then
        echoContent skyBlue "\nProgress $1/${totalProgress} : Adding scheduled maintenance certificate"
        ...
        echoContent green "\n ---> Successfully added scheduled maintenance for certificates"
    fi
}
# 定时任务更新geo文件
installCronUpdateGeo() {
    if [[ -n "${configPath}" ]]; then
        if crontab -l | grep -q "UpdateGeo"; then
            echoContent red "\n ---> Automatic update scheduled task has been added, please do not add it again"
            exit 0
        fi
        echoContent skyBlue "\nProgress 1/1: Adding scheduled update of geo file"
        crontab -l >/etc/v2ray-agent/backup_crontab.cron
        ...
        crontab /etc/v2ray-agent/backup_crontab.cron
        echoContent green "\n ---> Successfully added scheduled update of geo files"
    fi
}
...
renewalTLS() {
    ...
    if [[ -n $1 ]]; then
        echoContent skyBlue "\nProgress $1/1: Updating certificate"
    fi
    readAcmeTLS
    ...
    if [[ -d "$HOME/.acme.sh/${domain}_ecc" && -f "$HOME/.acme.sh/${domain}_ecc/${domain}.key" && -f "$HOME/.acme.sh/${domain}_ecc/${domain}.cer" ]] || [[ "${installDNSACMEStatus}" == "true" ]]; then
        ...
        echoContent skyBlue " ---> Automatically update on the last day before the certificate expires. If the update fails, please update manually."
...
installV2Ray() {
    readInstallType
    echoContent skyBlue "\nProgress  $1/${totalProgress} : Installing V2Ray"
    ...
    if [[ "${coreInstallType}" != "2" && "${coreInstallType}" != "3" ]]; then
        ...
        if [[ "${selectCoreType}" == "3" ]]; then
            ...
            read -r -p "Whether to update or upgrade? [y/n]:" reInstallV2RayStatus
...
installTuic() {
    readInstallType
    echoContent skyBlue "\nProgress  $1/${totalProgress} : Installing Tuic"
    ...
    if [[ -z "${tuicConfigPath}" ]]; then
        ...
        read -r -p "Do you want to update or upgrade? [y/n]:" reInstallTuicStatus
...
installXray() {
    ...
    if [[ "${coreInstallType}" != "1" ]]; then
        ...
        if [[ ! -f "/etc/v2ray-agent/xray/${xrayCoreCPUVendor}.zip" ]]; then
            echoContent red " ---> Core download failed, please try installing again"
            exit 0
        fi
        ...
        read -r -p "Whether to update and upgrade? [y/n]:" reInstallXrayStatus
...
v2rayVersionManageMenu() {
    ...
    if [[ ! -d "/etc/v2ray-agent/v2ray/" ]]; then
        echoContent red " ---> No installation directory detected. Please run the script to install the content."
        menu
        exit 0
    fi
    echoContent red "\n=============================================================="
    ...
    echoContent yellow "6. Update geosite and geoip"
    echoContent yellow "7. Set up automatic updating of the geo file [updated every day at midnight]"
    echoContent red "=============================================================="
    ...
    if [[ "${selectV2RayType}" == "1" ]]; then
        ...
        echoContent yellow "2. No guarantee that normal operation can be ensured after rollback."
        echoContent yellow "3. If the rolled back version does not support the current config, the connection will not be possible. Please proceed with caution."
        echoContent skyBlue "------------------------Version-------------------------------"
...
xrayVersionManageMenu() {
    ...
    echoContent yellow "7. Update geosite and geoip"
    echoContent yellow "8. Set up automatic updating of geo files [updated every day at midnight]"
    echoContent red "=============================================================="
    ...
    if [[ "${selectXrayType}" == "1" ]]; then
        ...
        echoContent skyBlue "--------------------------------------------------------------"
        read -r -p "Please enter the version to rollback:" selectXrayVersionType
...
updateV2Ray() {
    ...
    if [[ -z "${coreInstallType}" ]]; then
        ...
        if [[ -n "$1" ]]; then
            read -r -p "Roll back version to ${version}, continue? [y/n]:" rollbackV2RayStatus
            if [[ "${rollbackV2RayStatus}" == "y" ]]; then
                ...
                echoContent green " ---> Abandon rollback version"
            fi
...
updateXray() {
    ...
    if [[ -z "${coreInstallType}" ]]; then
        ...
        if [[ -n "$1" ]]; then
            read -r -p "Rollback version to ${version}, continue? [y/n]:" rollbackXrayStatus
...
checkGFWStatue() {
    readInstallType
    echoContent skyBlue "\nProgress $1/${totalProgress} : Verifying service startup status"
    if [[ "${coreInstallType}" == "1" ]] && [[ -n $(pgrep -f "xray/xray") ]]; then
        echoContent green " ---> Service started successfully"
    elif [[ "${coreInstallType}" == "2" ]] && [[ -n $(pgrep -f "v2ray/v2ray") ]]; then
        echoContent green " ---> Service started successfully"
    else
        echoContent red " ---> Service start failed, please check if there are any log prints in the terminal"
        exit 0
    fi
...
# V2Ray开机自启
installV2RayService() {
    echoContent skyBlue "\nProgress $1/${totalProgress}: Configuring V2Ray to start on boot"
...
EOF
        ...
        echoContent green " ---> Successfully configured V2Ray to start automatically on boot"
    fi
}

# 安装hysteria开机自启
installHysteriaService() {
    echoContent skyBlue "\nProgress $1/${totalProgress}: Configuring Hysteria to start on boot"
    if [[ -n $(find /bin /usr/bin -name "systemctl") ]]; then
...
EOF
        ...
        echoContent green " ---> Successfully configured Hysteria to start on boot"
    fi
}
# 安装Tuic开机自启动
installTuicService() {
    echoContent skyBlue "\nProgress  $1/${totalProgress} : Configuring Tuic to start on boot"
    if [[ -n $(find /bin /usr/bin -name "systemctl") ]]; then
...
EOF
        ...
        echoContent green " ---> Successfully configured Tuic to start automatically at boot"
    fi
}
# Xray开机自启
installXrayService() {
    echoContent skyBlue "\nProgress  $1/${totalProgress} : Configuring Xray to start on boot"
    if [[ -n $(find /bin /usr/bin -name "systemctl") ]]; then
...
handleV2Ray() {
    ...
    if [[ "$1" == "start" ]]; then
        ...
        if [[ -z $(pgrep -f "v2ray/v2ray") ]]; then
            echoContent green " ---> V2Ray successfully closed"
        else
            echoContent red "V2Ray failed to close"
            echoContent red "Please manually execute the command 【ps -ef|grep -v grep|grep v2ray|awk '{print \$2}'|xargs kill -9】"
            exit 0
        fi
    fi
}
...
handleHysteria() {
    ...
    if [[ "$1" == "start" ]]; then
        if [[ -n $(pgrep -f "hysteria/hysteria") ]]; then
            ...
            echoContent red "Hysteria启动失败" translates to "Hysteria failed to start" in English.
            echoContent red "Please manually execute 【/etc/v2ray-agent/hysteria/hysteria --log-level debug -c /etc/v2ray-agent/hysteria/conf/config.json server】, and check the error log."
            exit 0
        fi
    elif [[ "$1" == "stop" ]]; then
        if [[ -z $(pgrep -f "hysteria/hysteria") ]]; then
            ...
            echoContent red "Hysteria close failed"
...
handleTuic() {
    ...
    if [[ "$1" == "start" ]]; then
        if [[ -n $(pgrep -f "tuic/tuic") ]]; then
            ...
            echoContent red "Tuic failed to start"
        ...
        if [[ -z $(pgrep -f "tuic/tuic") ]]; then
            ...
            echoContent red "Please manually execute the command 【ps -ef|grep -v grep|grep tuic|awk '{print \$2}'|xargs kill -9】"
            exit 0
        fi
    fi
}
...
handleXray() {
    ...
    if [[ "$1" == "start" ]]; then
        if [[ -n $(pgrep -f "xray/xray") ]]; then
            ...
            echoContent red "Please manually execute the following command: [/etc/v2ray-agent/xray/xray -confdir /etc/v2ray-agent/xray/conf] and provide feedback on the error logs."
            exit 0
        fi
    elif [[ "$1" == "stop" ]]; then
        if [[ -z $(pgrep -f "xray/xray") ]]; then
            ...
            echoContent red "Please manually execute the command: 【ps -ef|grep -v grep|grep xray|awk '{print \$2}'|xargs kill -9】"
            exit 0
        fi
    fi
}
...
getClients() {
    ...
    if [[ ${addClientsStatus} == "true" ]]; then
        if [[ ! -f "${path}" ]]; then
            ...
            echoContent yellow "The configuration file from the previous installation on this protocol [${protocol}] was not found, using the first UUID from the configuration file."
        else
...
initHysteriaPort() {
    ...
    if [[ -n "${hysteriaPort}" ]]; then
        read -r -p "Read the port used during the last installation. Do you want to use the port used during the last installation? [y/n]:" historyHysteriaPortStatus
    ...
    if [[ -z "${hysteriaPort}" ]]; then
        echoContent yellow "Please enter the Hysteria port [press Enter for a random port between 10000-30000], it must not be the same as any other service."
...
initHysteriaNetwork() {
    ...
    echoContent yellow "Please enter the average latency from local to server, please fill in according to the actual situation (default: 180, unit: ms)"
    ...
    echoContent yellow "Please enter the maximum downstream speed of the local bandwidth (default: 100, unit: Mbps)"
    ...
    echoContent yellow "Please enter the upload speed of the local peak bandwidth (default: 50, unit: Mbps)"
...
hysteriaPortHopping() {
    if [[ -n "${portHoppingStart}" || -n "${portHoppingEnd}" ]]; then
        echoContent red " ---> Already added, cannot be added again. Please remove and add again."
        exit 0
    fi
    ...
    echoContent yellow "The end position of the port hopping is 60000."
    echoContent yellow "You can select a range within 30000-60000."
    echoContent yellow "Suggest around 1000"
    ...
    if [[ -z "${hysteriaPortHoppingRange}" ]]; then
        ...
        if [[ -z "${portStart}" || -z "${portEnd}" ]]; then
            ...
            if iptables-save | grep -q "mack-a_portHopping"; then
                ...
                echoContent red " ---> Port hopping addition failed"
            fi
            #            fi
        fi
...
hysteriaPortHoppingMenu() {
    ...
    if ! find /usr/bin /usr/sbin | grep -q -w iptables; then
        echoContent red " ---> Unable to recognize the iptables tool, unable to use port forwarding, exiting installation"
        exit 0
    fi
    readHysteriaConfig
    readHysteriaPortHopping
...
initTuicPort() {
    readTuicConfig
    if [[ -n "${tuicPort}" ]]; then
        read -r -p "The port used during the last installation is detected. Do you want to use the same port as the last installation? [y/n]:" historyTuicPortStatus
    ...
    if [[ -z "${tuicPort}" ]]; then
        echoContent yellow "Please enter the Tuic port [press Enter for a random port between 10000-30000], it must not conflict with other services."
...
tuicCoreInstall() {
    if ! echo "${currentInstallProtocolType}" | grep -q "0" || [[ -z "${coreInstallType}" ]]; then
        echoContent red "\n ---> Due to environment dependencies, such as installing Tuic, please install Xray-core's VLESS_TCP_TLS_Vision first."
        exit 0
    fi
    totalProgress=5
    installTuic 1
...
initV2RayConfig() {
    ...
    if [[ -n "${currentUUID}" && -z "${uuid}" ]]; then
        read -r -p "Read last installation record, do you want to use the UUID from the previous installation? [y/n]:" historyUUIDStatus
...
initXrayFrontingConfig() {
    ...
    if [[ "${coreInstallType}" != "1" ]]; then
        echoContent red " ---> Uninstalled available type"
    fi
    local xtlsType=
    if echo ${currentInstallProtocolType} | grep -q trojan; then
        xtlsType=VLESS
    ...
    echoContent skyBlue "\nFunction 1/${totalProgress}: Switching to ${xtlsType} as the prerequisite."
    echoContent red "\n=============================================================="
    echoContent yellow "# Precautions\n"
    echoContent yellow "Replace the prefix with ${xtlsType}"
    echoContent yellow "If the front-end is Trojan, when checking the account, there will be two Trojan protocol nodes, one of which is unavailable xtls."
    echoContent yellow "Executing again will switch to the previous foreground process."
...
initXrayConfig() {
    ...
    if [[ -n "${currentUUID}" ]]; then
        read -r -p "Read last user configuration, do you want to use the last installation configuration? [y/n]:" historyUUIDStatus
        if [[ "${historyUUIDStatus}" == "y" ]]; then
            addClientsStatus=true
            echoContent green "\n ---> Successful use"
        fi
    ...
    if [[ -z "${addClientsStatus}" ]]; then
        echoContent yellow "Please enter a custom UUID [must be valid], press [Enter] for a random UUID."
    ...
    if [[ -z "${addClientsStatus}" && -z "${uuid}" ]]; then
        addClientsStatus=
        echoContent red "\n ---> UUID read error, generating random"
        uuid=$(/etc/v2ray-agent/xray/xray uuid)
    fi
...
# 自定义CDN IP
customCDNIP() {
    echoContent skyBlue "\nProgress $1/${totalProgress} : Adding cloudflare custom CNAME"
    echoContent red "\n=============================================================="
    echoContent yellow "# Important notes"
    echoContent yellow "\nTutorial address:"
    echoContent skyBlue "https://www.v2ray-agent.com/archives/cloudflarezi-xuan-ip"
    echoContent red "\nIf you are not familiar with optimizing Cloudflare, please do not use it."
    echoContent yellow "\n 1.CNAME www.digitalocean.com"
    echoContent yellow " 2.CNAME who.int"
    echoContent yellow " 3.CNAME blog.hostmonit.com"

    echoContent skyBlue "----------------------------"
    read -r -p "Please select [press Enter to not use]:" selectCloudflareType
...
defaultBase64Code() {
    ...
    if [[ "${type}" == "vlesstcp" ]]; then
        ...
        if [[ "${coreInstallType}" == "1" ]] && echo "${currentInstallProtocolType}" | grep -q 0; then
            echoContent yellow " ---> General format (VLESS+TCP+TLS_Vision)"
            ...
            echoContent yellow " ---> Format plaintext (VLESS+TCP+TLS_Vision)"
...
EOF
            ...
            echoContent yellow " ---> Format plaintext (VLESS+TCP+TLS)"
...
EOF
        ...
        echoContent yellow " ---> Universal format (Trojan+TCP+TLS_Vision)"
        ...
        echoContent yellow " ---> Format plaintext (Trojan+TCP+TLS_Vision)"
...
EOF
        ...
        echoContent green "Protocol type: VLESS, Address: ${add}, Fake domain/SNI: ${currentHost}, Port: ${currentDefaultPort}, Client fingerprint: chrome, User ID: ${id}, Security: tls, Transmission method: ws, Path: /${currentPath}ws, Account name: ${email}\n"
...
EOF
        ...
        echoContent green "Protocol type: VLESS, Address: ${add}, Obfuscated domain/SNI: ${currentHost}, Port: ${currentDefaultPort}, User ID: ${id}, Security: tls, Transport method: gRPC, ALPN: h2, Client fingerprint: chrome, Service name: ${currentPath}grpc, Account name: ${email}\n"
...
EOF
        ...
        echoContent yellow " ---> Format plaintext (VLESS+reality+uTLS+Vision)"
        echoContent green "Protocol type: VLESS reality, Address: $(getPublicIP), PublicKey: ${currentRealityPublicKey}, Server names: ${currentRealityServerNames}, Port: ${currentRealityPort}, User ID: ${id}, Transmission method: tcp, Account name: ${email}\n"
        cat <<EOF >>"/etc/v2ray-agent/subscribe_local/default/${user}"
...
EOF
        ...
        echoContent yellow " ---> Format plaintext (VLESS+reality+uTLS+gRPC)"
        echoContent green "Protocol type: VLESS reality, serviceName: grpc, address: $(getPublicIP), publicKey: ${currentRealityPublicKey}, serverNames: ${currentRealityServerNames}, port: ${currentRealityPort}, userID: ${id}, transport method: gRPC, client-fingerprint: chrome, account name: ${email}\n"
        cat <<EOF >>"/etc/v2ray-agent/subscribe_local/default/${user}"
...
EOF
        ...
        echoContent yellow " ---> Format plaintext (Tuic+TLS)"
...
showAccounts() {
    ...
    if echo ${currentInstallProtocolType} | grep -q 6; then
        echoContent skyBlue "\n================================  Hysteria TLS  ================================\n"
        echoContent red "\n ---> Hysteria speed depends on the local network environment. If it is used with QoS, the experience will be very poor. IDC may also consider it as an attack, so please use it with caution."
    ...
    if echo ${currentInstallProtocolType} | grep -q 9; then
        echoContent skyBlue "\n================================  Tuic TLS  ================================\n"
        echoContent yellow "\n --->Tuic compared to Hysteria will be warmer and the user experience may be smoother."
...
checkNginx302() {
    ...
    echoContent red " ---> 302 redirection setting failed, please carefully check if it is the same as the example"
...
backupNginxConfig() {
    ...
    if [[ "$1" == "restoreBackup" ]] && [[ -f "/etc/v2ray-agent/alone_backup.conf" ]]; then
        ...
        echoContent green " ---> nginx configuration file backup restored successfully"
...
addNginx302() {
    ...
    grep -n "Strict-Transport-Security" <"${nginxConfigPath}alone.conf" | while read -r line; do
        if [[ -n "${line}" ]]; then
            ...
            echoContent red " ---> 302 addition failed"
            backupNginxConfig restoreBackup
        fi
...
# 更新伪装站
updateNginxBlog() {
    echoContent skyBlue "\nProgress $1/${totalProgress} : Changing camouflage site"

    if ! echo "${currentInstallProtocolType}" | grep -q "0" || [[ -z "${coreInstallType}" ]]; then
        echoContent red "\n ---> Due to environment dependencies, please install Xray-core's VLESS_TCP_TLS_Vision first."
        exit 0
    fi
    echoContent red "=============================================================="
    echoContent yellow "# If you need to customize, please manually copy the template file to ${nginxStaticPath} \n"
    echoContent yellow "1. New user guide"
    ...
    echoContent yellow "9.404 automatically redirects to Baidu"
    ...
    if [[ "${selectInstallNginxBlogType}" == "10" ]]; then
        echoContent red "\n=============================================================="
        echoContent yellow "The priority of redirection is higher. After configuring 302, if the disguised site is changed, the disguised site under the root route will not work."
        echoContent yellow "To disguise the site and achieve the desired effect, you need to delete the 302 redirection configuration."
        ...
        if [[ "${redirectStatus}" == "2" ]]; then
            removeNginx302
            echoContent green " ---> Successfully removed 302 redirect"
            exit 0
        fi
    fi
    if [[ "${selectInstallNginxBlogType}" =~ ^[1-9]$ ]]; then
        ...
        echoContent green " ---> Successfully replaced the fake website"
    else
        echoContent red " ---> Selection error, please choose again"
        updateNginxBlog
    fi
...
addCorePort() {
    ...
    echoContent yellow "Does not affect the use of default ports"
    echoContent yellow "When viewing accounts, only accounts with the default port will be displayed."
    echoContent yellow "No special characters allowed, pay attention to the format of commas."
    echoContent yellow "If hysteria is already installed, the new hysteria port will be installed simultaneously."
    echoContent yellow "Input example: 2053, 2083, 2087\n"

    echoContent yellow "1. View the added ports"
    ...
    if [[ "${selectNewPortType}" == "1" ]]; then
        ...
        read -r -p "Please enter the default port number, which will also change the subscription port and node port, [Enter] default 443:" defaultPort
...
EOF
        ...
        find ${configPath} -name "*dokodemodoor*" | grep -v "hysteria" | awk -F "[c][o][n][f][/]" '{print $2}' | awk -F "[_]" '{print $4}' | awk -F "[.]" '{print ""NR""":"$1}'
        read -r -p "Please enter the port index you want to delete:" portIndex
        local dokoConfig
...
# 卸载脚本
unInstall() {
    read -r -p "Are you sure you want to uninstall the installed content? [y/n]:" unInstallStatus
    if [[ "${unInstallStatus}" != "y" ]]; then
        echoContent green " ---> Abandon uninstallation"
        menu
        exit 0
    fi
    echoContent yellow " ---> The script will not delete any acme-related configurations. If you want to delete them, please manually execute [rm -rf /root/.acme.sh]"
    handleNginx stop
    if [[ -z $(pgrep -f "nginx") ]]; then
    ...
    if [[ "${coreInstallType}" == "1" ]]; then
        handleXray stop
        rm -rf /etc/systemd/system/xray.service
        echoContent green " ---> Xray startup deletion completed"

    elif [[ "${coreInstallType}" == "2" ]]; then

        handleV2Ray stop
        rm -rf /etc/systemd/system/v2ray.service
        echoContent green " ---> V2Ray startup auto-enable deleted"
    ...
    if [[ -z "${hysteriaConfigPath}" ]]; then
        handleHysteria stop
        rm -rf /etc/systemd/system/hysteria.service
        echoContent green " ---> Successfully removed Hysteria from startup"
    fi

    if [[ -z "${tuicConfigPath}" ]]; then
        handleTuic stop
        rm -rf /etc/systemd/system/tuic.service
        echoContent green " ---> Delete Tuic startup completed"
    fi
    ...
    if [[ -d "${nginxStaticPath}" && -f "${nginxStaticPath}/check" ]]; then
        rm -rf "${nginxStaticPath}"
        echoContent green " ---> Deletion of disguised website completed"
    fi
...
updateV2RayCDN() {
    ...
    echoContent skyBlue "\nProgress $1/${totalProgress}: Modifying CDN node"
    ...
    if [[ -n "${currentAdd}" ]]; then
        ...
        echoContent yellow "2.CNAME who.int"
        echoContent yellow "3.CNAME blog.hostmonit.com"
        echoContent yellow "4. Manually enter [can enter multiple, for example: 1.1.1.1,1.1.2.2,cloudflare.com separated by commas]"
        ...
        3)
            ...
            ;;
        4)
            read -r -p "Please enter the custom CDN IP or domain name:" setDomain
            ;;
        5)
        ...
        if [[ -n "${setDomain}" ]]; then
            ...
            echoContent green " ---> Successfully modified CDN"
        fi
    else
        echoContent red " ---> Uninstalled available type"
    fi
...
# 自定义uuid
customUUID() {
    read -r -p "Please enter a valid UUID, [Enter] for a random UUID:" currentCustomUUID
    echo
    if [[ -z "${currentCustomUUID}" ]]; then
        ...
        if [[ -f "/tmp/v2ray-agent" && -n $(cat /tmp/v2ray-agent) ]]; then
            echoContent red " ---> UUID cannot be duplicated"
            rm /tmp/v2ray-agent
            exit 0
        fi
    fi
...
# 自定义email
customUserEmail() {
    read -r -p "Please enter a valid email, [Enter] for a random email:" currentCustomEmail
    echo
    if [[ -z "${currentCustomEmail}" ]]; then
        ...
        if [[ -f "/tmp/v2ray-agent" && -n $(cat /tmp/v2ray-agent) ]]; then
            echoContent red " ---> email cannot be duplicated"
            rm /tmp/v2ray-agent
            exit 0
        fi
    fi
...
addUserXray() {
    ...
    if [[ -z ${userNum} || ${userNum} -le 0 ]]; then
        echoContent red " ---> Input error, please enter again"
        exit 0
    fi
    # 生成用户
    if [[ "${userNum}" == "1" ]]; then
    ...
    reloadCore
    echoContent green " ---> Addition completed"
    manageAccount 1
}
# 添加用户
addUser() {

    echoContent yellow "After adding a new user, you need to view the subscription again."
    read -r -p "Please enter the number of users to be added:" userNum
    echo
    if [[ -z ${userNum} || ${userNum} -le 0 ]]; then
        echoContent red " ---> Input error, please enter again"
...
# 更新脚本
updateV2RayAgent() {
    echoContent skyBlue "\nProgress $1/${totalProgress}: Updating v2ray-agent script"
...
handleFirewall() {
    if systemctl status ufw 2>/dev/null | grep -q "active (exited)" && [[ "$1" == "stop" ]]; then
        ...
        echoContent green " ---> ufw successfully closed"
    ...
    if systemctl status firewalld 2>/dev/null | grep -q "active (running)" && [[ "$1" == "stop" ]]; then
        ...
        echoContent green " ---> firewalld successfully closed"
    fi
}
...
bbrInstall() {
    echoContent red "\n=============================================================="
    echoContent green "BBR, DD script uses the mature work of [ylx2016], address [https://github.com/ylx2016/Linux-NetSpeed], please take note."
...
checkLog() {
    ...
    echoContent red "\n=============================================================="
    echoContent yellow "# It is recommended to only enable access logs during debugging\n"

    if [[ "${logStatus}" == "false" ]]; then
        echoContent yellow "1. Open access log"
    else
        echoContent yellow "1. Disable access logs"
    fi

    echoContent yellow "2. Monitor access logs"
    echoContent yellow "3. Monitor error logs"
    echoContent yellow "4. View certificate scheduled task logs"
    echoContent yellow "5. View certificate installation log"
    echoContent yellow "6. Clear logs"
    echoContent red "=============================================================="
...
aliasInstall() {
    ...
    if [[ -f "$HOME/install.sh" ]] && [[ -d "/etc/v2ray-agent" ]] && grep <"$HOME/install.sh" -q "作者:mack-a"; then
        ...
        if [[ "${vasmaType}" == "true" ]]; then
            echoContent green "Shortcut created successfully. You can run [vasma] to reopen the script."
        fi
    fi
}
...
ipv6Routing() {
    ...
    checkIPv6
    echoContent skyBlue "\nFunction 1/${totalProgress}: IPv6 Traffic Shaping"
    echoContent red "\n=============================================================="
    echoContent yellow "1. View the redirected domain names"
    echoContent yellow "2. Add domain name"
    echoContent yellow "3. Set IPv6 global"
    echoContent yellow "4. Uninstall IPv6 routing"
    echoContent red "=============================================================="
    ...
    if [[ "${ipv6Status}" == "1" ]]; then
        ...
        echoContent yellow "# Precautions\n"
        echoContent yellow "# Important notes"
        echoContent yellow "# Usage tutorial: https://www.v2ray-agent.com/archives/ba-he-yi-jiao-ben-yu-ming-fen-liu-jiao-cheng \n"
        ...
        echoContent red "=============================================================="
        echoContent yellow "# Precautions\n"
        echoContent yellow "1. Will delete all set routing rules."
        echoContent yellow "2. It will delete all outbound rules except for IPv6."
        read -r -p "Are you sure you want to set it? [y/n]:" IPv6OutStatus
...
EOF
            ...
            echoContent green " ---> IPv6 global outbound settings successfully configured"
        else
            echoContent green " ---> Abandon setup"
            exit 0
        fi
        ...
        echoContent green " ---> IPv6 routing uninstalled successfully"
    else
        echoContent red " ---> Wrong selection"
        exit 0
    fi
...
btTools() {
    ...
    echoContent skyBlue "\nFeature 1/${totalProgress}: bt download management"
    echoContent red "\n=============================================================="
...
blacklist() {
    ...
    if [[ "${blacklistStatus}" == "1" ]]; then
        ...
        echoContent red "=============================================================="
        echoContent yellow "# Precautions\n"
        echoContent yellow "1. Rule supports pre-defined domain name list [https://github.com/v2fly/domain-list-community]"
        echoContent yellow "2. Rule supports custom domain names"
        echoContent yellow "3. Input examples: speedtest, facebook, cn, example.com"
        echoContent yellow "4. If the domain name exists in the predefined domain name list, use geosite:xx. If it does not exist, use the input domain name by default."
        echoContent yellow "5. Add rules as incremental configuration, without deleting previously set content."
        read -r -p "Please enter the domain name according to the example above:" domainList
        ...
        echoContent green " ---> Blocked domestic domain successfully"

    elif [[ "${blacklistStatus}" == "4" ]]; then

        unInstallRouting blackhole-out outboundTag

        echoContent green " ---> Domain blacklist deletion successful"
    else
        echoContent red " ---> Wrong selection"
        exit 0
    fi
...
EOF
    ...
    while read -r line; do
        if echo "${routingRule}" | grep -q "${line}"; then
            echoContent yellow " ---> ${line} already exists, skipping"
        else
            local geositeStatus
...
warpRouting() {
    ...
    if [[ -z $(which warp-cli) ]]; then
        echo
        read -r -p "WARP is not installed, do you want to install? [y/n]:" installCloudflareWarpStatus
    ...
    echoContent yellow "4. Uninstall WARP routing"
    echoContent red "=============================================================="
    ...
    if [[ "${warpStatus}" == "1" ]]; then
        ...
        echoContent yellow "# Precautions\n"
        echoContent yellow "1. Will delete all set routing rules."
        echoContent yellow "2. It will delete all outbound rules except for WARP."
        read -r -p "Are you sure you want to set it? [y/n]:" warpOutStatus
...
EOF
            rm ${configPath}09_routing.json >/dev/null 2>&1
            echoContent green " ---> WARP global outbound settings successful"
        else
            echoContent green " ---> Abandon setup"
            exit 0
        fi
        ...
        echoContent green " ---> WARP diversion uninstalled successfully"
    else
        echoContent red " ---> Wrong selection"
        exit 0
    fi
...
warpRoutingReg() {
    local type=$2
    echoContent skyBlue "\nProgress  $1/${totalProgress} : WARP Splitting [Third Party]"
    echoContent red "=============================================================="
    if [[ ! -f "/etc/v2ray-agent/warp/warp-reg" ]]; then
        echo
        echoContent yellow "# Important notes"
        echoContent yellow "# Dependencies on third-party programs, please be aware of the risks involved"
        echoContent yellow "# Project address: https://github.com/badafans/warp-reg \n"

        read -r -p "warp-reg is not installed, do you want to install? [y/n]:" installWarpRegStatus
    ...
    if [[ ${type} == "IPv4" ]]; then
        ...
        echoContent red " ---> Failed to obtain IP, exiting installation"
    fi
    ...
    if [[ "${warpStatus}" == "1" ]]; then
        ...
        echoContent yellow "# Precautions\n"
        echoContent yellow "1. Will delete all set routing rules."
        echoContent yellow "2. It will delete all outbound rules except for WARP [third-party]."
        read -r -p "Are you sure you want to set it? [y/n]:" warpOutStatus
...
# 分流工具
routingToolsMenu() {
    echoContent skyBlue "\nFeature 1/${totalProgress} : Traffic Splitting Tool"
    echoContent red "\n=============================================================="
    echoContent yellow "1. WARP traffic splitting [Third-party IPv4]"
    echoContent yellow "2. WARP routing [Third-party IPv6]"
    echoContent yellow "3. IPv6 routing/diversion"
    echoContent yellow "4. Arbitrary door diversion"
    echoContent yellow "5. DNS diversion"
    echoContent yellow "6. VMess+WS+TLS split"
    echoContent yellow "7. SNI reverse proxy shunting"
...
streamingToolbox() {
    ...
    echoContent red "\n=============================================================="
    echoContent yellow "1. Unlock streaming media with the landing machine of the Anydoor."
    echoContent yellow "2. DNS unlocking streaming media"
    echoContent yellow "3. VMess+WS+TLS unlocks streaming media"
    read -r -p "Please select:" selectType
...
# 任意门解锁流媒体
dokodemoDoorRouting() {
    echoContent skyBlue "\nFunction 1/${totalProgress} : Teleportation diversion"
    echoContent red "\n=============================================================="
    echoContent yellow "# Important notes"
    echoContent yellow "# Usage tutorial: https://www.v2ray-agent.com/archives/ba-he-yi-jiao-ben-yu-ming-fen-liu-jiao-cheng \n"
...
# VMess+WS+TLS 分流
vmessWSRouting() {
    echoContent skyBlue "\nFunction 1/${totalProgress} : VMess+WS+TLS Load Balancing"
    echoContent red "\n=============================================================="
    echoContent yellow "# Important notes"
    echoContent yellow "# Usage tutorial: https://www.v2ray-agent.com/archives/ba-he-yi-jiao-ben-yu-ming-fen-liu-jiao-cheng \n"
...
setVMessWSRoutingOutbounds() {
    ...
    echoContent red "=============================================================="
    echoContent yellow "Example input: netflix, openai\n"
    read -r -p "Please enter the domain name according to the example above:" domainList
    ...
    if [[ -n "${setVMessWSTLSAddress}" ]]; then
        ...
        read -r -p "Please enter the UUID for VMess+WS+TLS:" setVMessWSTLSUUID
        echo
        if [[ -z "${setVMessWSTLSUUID}" ]]; then
            echoContent red " ---> UUID cannot be empty"
        fi

        read -r -p "Please enter the Path for VMess+WS+TLS:" setVMessWSTLSPath
        echo
        ...
        reloadCore
        echoContent green " ---> Successfully added diversion"
        exit 0
    fi
...
# 设置任意门分流【出站】
setDokodemoDoorRoutingOutbounds() {
    read -r -p "Please enter the IP address of the target VPS:" setIP
    echoContent red "=============================================================="
    echoContent yellow "Example input: netflix, openai\n"
    read -r -p "Please enter the domain name according to the example above:" domainList
    ...
    if [[ -n "${setIP}" ]]; then
        ...
        reloadCore
        echoContent green " ---> Successfully added the diversion of the Anydoor"
        exit 0
    fi
    echoContent red " ---> IP address cannot be empty"
}

# 设置任意门分流【入站】
setDokodemoDoorRoutingInbounds() {

    echoContent skyBlue "\nFunction 1/${totalProgress}: Add inbound rule to the portal"
    echoContent red "\n=============================================================="
    echoContent yellow "IP input example: 1.1.1.1, 1.1.1.2"
    echoContent yellow "The domain below must match the outgoing VPS."
    echoContent yellow "Domain entry example: netflix, openai\n"
    read -r -p "Please enter the IP address allowed to access this VPS:" setIPs
    if [[ -n "${setIPs}" ]]; then
        read -r -p "Please enter the domain name according to the example above:" domainList
        allowPort 22387
        allowPort 22388
...
EOF
        ...
        reloadCore
        echoContent green " ---> Successfully added landing machine inbound diversion"
        exit 0
    fi
    echoContent red " ---> IP address cannot be empty"
}
...
sniRouting() {
    ...
    echoContent skyBlue "\nFeature 1/${totalProgress} : SNI reverse proxy load balancing"
    echoContent red "\n=============================================================="
    echoContent yellow "# Important notes"
    echoContent yellow "# Usage tutorial: https://www.v2ray-agent.com/archives/ba-he-yi-jiao-ben-yu-ming-fen-liu-jiao-cheng \n"
...
# 设置SNI分流
setUnlockSNI() {
    read -r -p "Please enter the SNI IP for routing:" setSNIP
    if [[ -n ${setSNIP} ]]; then
        echoContent red "=============================================================="
...
EOF
            echoContent red " ---> SNI reverse proxy routing successful"
            reloadCore
        else
            echoContent red " ---> Domain name cannot be empty"
        fi
...
# 设置dns
setUnlockDNS() {
    read -r -p "Please enter the DNS for the diversion:" setDNS
    if [[ -n ${setDNS} ]]; then
        echoContent red "=============================================================="
        echoContent yellow "Input example: netflix, disney, hulu"
        echoContent yellow "Please enter 1 for the default plan. The default plan includes the following contents."
...
EOF
        ...
        echoContent yellow "2. After uninstalling DNS unlock, modify the local DNS settings in [/etc/resolv.conf] and restart the VPS."
    else
...
customV2RayInstall() {
    ...
    echoContent yellow "VLESS front-end, default installation is 0. If you only need to install 0, just select 0."
    ...
    read -r -p "Please select [multiple choices], [example:123]:" selectCustomInstallType
    echoContent skyBlue "--------------------------------------------------------------"
...
customXrayInstall() {
    ...
    if [[ -z ${selectCustomInstallType} ]]; then
        ...
        installTools 1
        if [[ -n "${btDomain}" ]]; then
            echoContent skyBlue "\nProgress 3/${totalProgress} : Detected Baota panel, skipping TLS application step."
            handleXray stop
            customPortFunction
        else
            # 申请tls
        ...
        if [[ -n "${btDomain}" ]]; then
            echoContent skyBlue "\nProgress 6/${totalProgress}: Detected Baota panel, skipping disguised website"
            #            echoContent red "=============================================================="
...
selectCoreInstall() {
    ...
    2)
        v2rayCoreVersion=
        echoContent red " ---> Due to v2ray not supporting many new features and in order to reduce development costs, maintenance has been stopped. It is recommended to use Xray-core, hysteria, Tuic."
        exit 0
        if [[ "${selectInstallType}" == "2" ]]; then
            customV2RayInstall
        else
    ...
    3)
        ...
        ;;
    *)
        echoContent red ' ---> Wrong selection, please choose again'
        selectCoreInstall
        ;;
    esac
}
...
xrayCoreInstall() {
    ...
    installCronTLS 10
    if [[ -n "${btDomain}" ]]; then
        echoContent skyBlue "\nProgress 11/${totalProgress}: Detected Baota panel, skipping fake website disguise."
        #        echoContent red "=============================================================="
        #        echoContent yellow "# 注意事项"
        #        echoContent yellow "会清空当前安装网站下面的静态目录，如已自定义安装过请选择 [n]\n"
        #        read -r -p "请选择[y/n]:" nginxBlogBTStatus
...
hysteriaCoreInstall() {
    if ! echo "${currentInstallProtocolType}" | grep -q "0" || [[ -z "${coreInstallType}" ]]; then
        echoContent red "\n ---> Due to environment dependencies, such as installing hysteria, please install Xray-core's VLESS_TCP_TLS_Vision first."
        exit 0
    fi
    totalProgress=5
    installHysteria 1
...
unInstallXrayCoreReality() {
    ...
    echoContent skyBlue "\nFeature 1/1: Uninstalling reality"
    echoContent red "\n=============================================================="
    echoContent yellow "# Only delete VLESS Reality related configurations, other contents will not be deleted."
    echoContent yellow "# If you need to uninstall other content, please uninstall the script functionality"
    handleXray stop
...
manageAccount() {
    ...
    echoContent red "\n=============================================================="
    echoContent yellow "# Custom email and uuid can be added when adding a single user"
    echoContent yellow "# If Hysteria or Tuic is installed, the account will be added to the corresponding type as well"
...
addSubscribeMenu() {
    ...
    if [[ "${addSubscribeStatus}" == "1" ]]; then
        ...
        echoContent green " ---> Other machine subscription deleted successfully"
        subscribe
    fi
}
# 添加其他机器clashMeta订阅
addOtherSubscribe() {
    echoContent yellow "#Attention:"
    ...
    if [[ -z "${remoteSubscribeUrl}" ]]; then
        ...
        echoContent red " ---> The rule is not valid"
    else
        ...
        if [[ -n $(ls /etc/v2ray-agent/subscribe/clashMeta/) || -n $(ls /etc/v2ray-agent/subscribe/default/) ]]; then
            find /etc/v2ray-agent/subscribe_local/default/* | while read -r email; do
                ...
                if echo "${clashMetaProxies}" | grep -q "${email}"; then
                    ...
                    echoContent red " ---> The clashMeta subscription ${email} does not exist."
                fi
            done
        else
            echoContent red " ---> Please check the subscription first before adding a subscription."
        fi
    fi
}
# clashMeta配置文件
...
subscribe() {
    ...
    if echo "${currentInstallProtocolType}" | grep -q 0 && [[ -n "${configPath}" ]]; then
        ...
        echoContent yellow "# View the subscription that will regenerate the local account's subscription"
        echoContent yellow "# Adding or modifying an account requires rechecking the subscription to regenerate the subscription content for external access."
        echoContent red "# Need to manually enter the salt value for md5 encryption, if you don't understand, you can use random."
        echoContent yellow "# Does not affect the content of the remote subscription already added\n"

        if [[ -f "/etc/v2ray-agent/subscribe_local/subscribeSalt" && -n $(cat "/etc/v2ray-agent/subscribe_local/subscribeSalt") ]]; then
            read -r -p "Read the Salt settings from the previous installation. Do you want to use the previously generated Salt? [y/n]:" historySaltStatus
            if [[ "${historySaltStatus}" == "y" ]]; then
                subscribeSalt=$(cat /etc/v2ray-agent/subscribe_local/subscribeSalt)
            else
                read -r -p "Please enter the salt value, [Enter] to use random:" subscribeSalt
            fi
        else
            read -r -p "Please enter the salt value, [Enter] to use random:" subscribeSalt
        fi
        ...
        echoContent red " ---> Pseudo site not installed, unable to use subscription service"
    fi
}
...
updateRemoteSubscribe() {
    ...
    if [[ "${type}" == "ClashMeta" ]]; then
        ...
        if echo "${clashMetaProxies}" | grep -q "${email}"; then
            ...
            echoContent green " ---> clashMeta subscription to ${remoteDomain}:${email} updated successfully"
        else
        ...
        if echo "${default}" | grep -q "${email}"; then
            ...
            echoContent red " ---> The general subscription ${remoteDomain}:${email} does not exist"
        fi
    fi
}
...
# 切换alpn
switchAlpn() {
    echoContent skyBlue "\nFunction 1/${totalProgress} : Switching ALPN"
    ...
    echoContent red "\n=============================================================="
    echoContent green "The current ALPN is:${currentAlpn}"
    echoContent yellow "1. When http/1.1 is used as the first option, Trojan is available, and some gRPC clients are available [clients that support manual selection of available ALPN]."
    echoContent yellow "2. When h2 is at the beginning, gRPC is available, and some Trojan clients are available [clients support manual selection of available ALPN]."
    echoContent yellow "3. If the client does not support manually changing ALPN, it is recommended to use this feature to change the server's ALPN order in order to use the corresponding protocol."
    echoContent red "=============================================================="

    if [[ "${currentAlpn}" == "http/1.1" ]]; then
        echoContent yellow "1. Switch the first position of ALPN to h2"
    elif [[ "${currentAlpn}" == "h2" ]]; then
        echoContent yellow "1. Switching ALPN to HTTP/1.1 as the first option"
    else
        echoContent red '不符合' translates to 'does not comply' or 'not in accordance with' in English.
    fi
...
initRealityKey() {
    ...
    if [[ -n "${currentRealityPublicKey}" ]]; then
        read -r -p "Read the previous installation record, do you want to use the PublicKey/PrivateKey from the previous installation? [y/n]:" historyKeyStatus
...
checkRealityDest() {
    ...
    if [[ -n "${traceResult}" ]]; then
        echoContent red "\n ---> Detected the use of a domain hosted on Cloudflare with proxy enabled. Using this type of domain may result in VPS traffic being used by others [not recommended]\n"
        read -r -p "Continue? [y/n]" setRealityDestStatus
        if [[ "${setRealityDestStatus}" != 'y' ]]; then
            exit 0
        fi
        echoContent yellow "\n ---> Ignore the risk and continue using"
    fi
}
...
initRealityDest() {
    if [[ -n "${domain}" ]]; then
        ...
        echoContent skyBlue "\n===== Generate domain names for fallback configuration, for example: [addons.mozilla.org:443] ======\n"
        ...
        if ! echo "${realityDestDomain}" | grep -q ":"; then
            echoContent red "\n ---> Domain name is not in compliance, please enter again"
            initRealityDest
        else
            checkRealityDest
            echoContent yellow "\n ---> Falling back domain: ${realityDestDomain}"
        fi
    fi
}
...
initRealityClientServersName() {
    if [[ -n "${domain}" ]]; then
        ...
        echoContent skyBlue "\n================ Configure available serverNames for clients ================\n"
        echoContent yellow "#Attention"
        echoContent green List of serverNames available for the client: https://www.v2ray-agent.com/archives/1680104902581#heading-8\n
    ...
    echoContent yellow "\n ---> Client available domain name: ${realityServerNames}\n"
}
# 初始化reality端口
initRealityPort() {
    if [[ -n "${currentRealityPort}" ]]; then
        read -r -p "Read the last installation record, do you want to use the port from the previous installation? [y/n]:" historyRealityPortStatus
    ...
    if [[ -z "${realityPort}" ]]; then
        ...
        if [[ -z "${realityPort}" ]]; then
            echoContent yellow "Please enter the port number [Press Enter for a random port between 10000 and 30000]"
...
manageHysteria() {
    ...
    if [[ -n "${hysteriaConfigPath}" ]]; then
        echoContent yellow "1. Reinstall"
        echoContent yellow "2. Uninstall"
        echoContent yellow "3. Port hopping management"
        echoContent yellow "4.core management"
        echoContent yellow "5. View logs"
        hysteriaStatus=true
    else
        echoContent yellow "1. Install"
    fi
...
hysteriaVersionManageMenu() {
    echoContent skyBlue "\nProgress  $1/${totalProgress} : Hysteria version control"
...
tuicVersionManageMenu() {
    echoContent skyBlue "\nProgress  $1/${totalProgress} : Tuic version control"
...
menu() {
    ...
    echoContent green "Current version: v2.10.14"
    echoContent green "Github：https://github.com/mack-a/v2ray-agent"
    echoContent green "Description: Script for coexistence of eight entities\c"
    showInstallStatus
    checkWgetShowProgress
    echoContent red "\n=========================== Promotion Area ============================"
    echoContent red "                                              "
    echoContent green "Please contact TG for promotion: @mackaff\n"
    echoContent green "VPS purchase guide: https://www.v2ray-agent.com/archives/1679975663984"
    echoContent green "Yearly $10 low-price VPS AS4837: https://www.v2ray-agent.com/archives/racknerdtao-can-zheng-li-nian-fu-10mei-yuan"
    echoContent red "=============================================================="
    if [[ -n "${coreInstallType}" ]]; then
        echoContent yellow "1. Reinstall"
    else
    ...
    echoContent yellow "8. Change disguise site"
    ...
    echoContent skyBlue "-------------------------Version Control-----------------------------"
